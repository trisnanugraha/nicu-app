...
class PengajuanJadwalRuangan extends MY_Controller
{
    ...
    {
        $data['judul'] = 'Pengajuan Jadwal Ruangan';
        $data['ruangan'] = $this->Mod_ruangan->get_all_ruangan();
        // $data['manufaktur'] = $this->session->userdata('full_name');
        // $data['hakakses'] = $this->session->userdata('hak_akses');
        $data['modal_tambah'] = show_my_modal('pengajuan_jadwal_ruangan/modal_tambah_pengajuan_jadwal_ruangan', $data);
        $js = $this->load->view('pengajuan_jadwal_ruangan/pengajuan-jadwal-ruangan-js', null, true);
        $this->template->views('pengajuan_jadwal_ruangan/home', $data, $js);
    }

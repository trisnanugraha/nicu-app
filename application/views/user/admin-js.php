<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    table = $("#tabelAdmin").DataTable({
      ...
      "autoWidth": false,
      "language": {
        "sEmptyTable": "Data Admin Masih Kosong"
      },
  ...
  function del(id) {
    Swal.fire({
      ...
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Ya, Hapus Admin Ini!',
      cancelButtonText: 'Batal'
    }).then((result) => {
      if (result.value) {
        $.ajax({
  ...
  function add() {
    ...
    $('.modal-title').text('Tambah Admin Baru'); // Set Title to Bootstrap modal title
  }
      ...
      success: function (data) {
        ...
        if (data.status) //if success close modal and reload ajax table
        {
          ...
          if (save_method == 'add') {
            Toast.fire({
              icon: 'success',
              title: 'Admin Baru Berhasil Disimpan!'
            });
